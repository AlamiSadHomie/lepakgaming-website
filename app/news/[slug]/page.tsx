...
import { ArrowLeft, Calendar, User, Monitor, ExternalLink, Home } from "lucide-react";
...
export default async function NewsPage({ params }: PageProps) {
  ...
  const getBadgeText = (article: any) => (article.type === "original" ? "ORIGINAL NEWS" : `FROM ${article.source?.toUpperCase()}`);
  const categories = [
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <header className="bg-gray-800 border-b border-gray-700">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 py-3 sm:py-0 h-auto sm:h-16 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav className="flex flex-wrap justify-center gap-2 text-sm w-full sm:flex-1">
              {categories.map((cat) => (
                <Link
                  key={cat.id}
                  href={cat.href}
                  className={`px-3 py-2 rounded-md font-medium transition ${
                    cat.id === "news" ? "bg-purple-600 text-white" : "text-gray-300 hover:bg-gray-700"
                  }`}
                >
